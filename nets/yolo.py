...
class YoloBody(nn.Module):
    def __init__(self, anchors_mask, num_classes, backbone="mobilenetv2", pretrained=False):
        ...
        if backbone == "mobilenetv1":
            ...
            #---------------------------------------------------#   
            #   104,104,24;52,52,40；26,26,112；13,13,160
            #---------------------------------------------------#
            self.backbone   = GhostNet(pretrained=pretrained)
            in_filters      = [24, 40, 112, 160]
        elif backbone == "vgg":
            #---------------------------------------------------#   
            ...
            #---------------------------------------------------#   
            #   104,104,256;52,52,512；26,26,1024；13,13,2048
            #---------------------------------------------------#
        ...
        self.conv1           = make_three_conv([512, 1024], in_filters[3])
        self.SPP             = SpatialPyramidPooling()
        self.conv2           = make_three_conv([512, 1024], 2048)

        self.upsample1       = Upsample(512, 256)
        self.conv_for_P4     = conv2d(in_filters[2], 256,1)
        self.make_five_conv1 = make_five_conv([256, 512], 512)

        self.upsample2       = Upsample(256, 128)
        self.conv_for_P3     = conv2d(in_filters[1], 128,1)
        self.make_five_conv2 = make_five_conv([128, 256], 256)

        self.upsample3       = Upsample(128, 64)
        self.conv_for_P2     = conv2d(in_filters[0], 64,1)
        self.make_five_conv3 = make_five_conv([64, 128], 128)

        # 3*(5+num_classes) = 3*(5+20) = 3*(4+1+20)=75
        self.yolo_head4      = yolo_head([128, len(anchors_mask[0]) * (5 + num_classes)], 64)

        self.down_sample1    = conv_dw(64, 128, stride = 2)
        self.make_five_conv4 = make_five_conv([128, 256], 256)

        self.yolo_head3 = yolo_head([256, len(anchors_mask[0]) * (5 + num_classes)], 128)

        self.down_sample2    = conv_dw(128, 256, stride = 2)
        self.make_five_conv5 = make_five_conv([256, 512], 512)

        # 3*(5+num_classes) = 3*(5+20) = 3*(4+1+20)=75
        self.yolo_head2      = yolo_head([512, len(anchors_mask[1]) * (5 + num_classes)], 256)

        self.down_sample3    = conv_dw(256, 512, stride = 2)
        self.make_five_conv6 = make_five_conv([512, 1024], 1024)
