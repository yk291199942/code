...
def hazy_simu(img_name, depth_or_trans_name, airlight=0.76, is_imdepth=1):  ##for outdoor
    ...
    if is_imdepth:
        ...
        transmission_3 = np.array([transmission]).reshape(3,915,1044)
...
f=[]
for file in files:
    poy=os.path.splitext(file)
    f.append(poy[0])
print(f)


for fil in f:
    path11=os.path.join(path1,fil+'.jpg')
    path22=os.path.join(path2,fil+'.npy')
    ...
    for j in range(1, 3):
        ...
        img_haze,  airlight, beta_return = hazy_reside_OTS(img, npy)
        airlight = round(airlight, 2)
        beta_return = round(beta_return, 2)

            # Image.Image.show(img)
        path_save = os.path.join(path3, fil +'.png')
